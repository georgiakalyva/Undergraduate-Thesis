    ...
    sealed partial class App : Application
    {
        ...
        private async void InitNotificationsAsync()
        {
            try
            {
                ...
                {
                    NotificationsEnabled = Convert.ToBoolean(db.Table<Setting>().Where(x => x.ID == "Notifications").FirstOrDefault().Description);
                }
            }
            catch (Exception)
            {
            ...
            {
                MessageDialog showDialog = new MessageDialog("This app uses notifications, do you wish to enable them?");
                ...
                var dialogresult = await showDialog.ShowAsync();
                ...
                if ((int)dialogresult.Id == 0)
                 {
                    ...
                    using (var db = SqliteDatabase.DBConnection)
                    {
                        db.InsertOrReplace(new Setting() { ID = "Notifications", Description = true.ToString() });
                    }
                ...
                else
                {
                    using (var db = SqliteDatabase.DBConnection)
                    {
                        db.InsertOrReplace(new Setting() { ID = "Notifications", Description = false.ToString() });
                    }
                }
            }
            else if ((bool)NotificationsEnabled)
            {
                var channel = await PushNotificationChannelManager.CreatePushNotificationChannelForApplicationAsync();
